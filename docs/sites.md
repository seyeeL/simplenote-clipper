...
Valir Masha(8月24日): Open Source: lersent001.github.io/orb/
```

一条回复都没有时抬头也不出现，笔记末尾不会挂一句空的 `comments:`。

**一条回复拼成一行，不整块收 `<article>`。** 整块收的第一版实测下来不能看：昵称、`@handle`、
时间、正文、阅读数各占一行，一条回复七八行，十几条读下来全是碎片，阅读数（`11K`、`7.2K`）
还混在里面像是正文的一部分。现在只挑昵称、时间、正文、配图四样，规则自己拼出
`昵称(时间): 正文` 这一行，其余（头像、`@handle`、阅读数、转推收藏那排按钮）一概不要。
配图跟在这一行后面另起，多行的回复保留它自己的换行。

拼行的代码在 `lib/site-rules.js` 的 `formatReply`：`replies` 返回的不一定是页面上的现成元素，
自己 `createElement` 拼一个出来也行，引擎照样把它接进正文。正文搬的是子节点不是整块 —— 正文在
页面上是个 `div`，整块塞进这一行会自成一段，`昵称(时间): ` 就孤零零留在上一行了。

**只收页面上已经加载出来的回复。** X 的回复列表是滚动加载的，没往下翻就只有头几条；翻得太远的
反过来会被回收出 DOM。想多剪几条就先在页面上翻到，再点剪藏。

**回复只算主推底下的。** 主推之前的是上文（点开串里的第二条时会看到），不进笔记；回复列表底下
接着的「更多推荐」在对话容器之外，也不进。
...
3. 往 `lib/site-rules.js` 加一条。字段都是可选的，只写通用逻辑搞不定的那几项：
   ...
   自己 `doc.createElement` 拼（推特就是这么把一条回复拼成一行的）。
