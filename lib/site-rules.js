...
// 回复正文搬进合成的那一行之后，靠这个类名让换行还能转成 <br>
const REPLY_BODY_CLASS = 'x-reply-body';
// 评论那一段的抬头，跟在分隔线后面
const REPLIES_LABEL = 'comments:';
...
/**
 * 一条回复拼成「昵称(时间): 正文」这一行，配图跟在后面另起。
 *
 * 不整块收 article：那样昵称、@handle、时间、正文、阅读数各占一行，十几条回复读下来
 * 全是碎片。这里只挑昵称、时间、正文、配图四样，其余（头像、@handle、阅读数、
 * 转推收藏那排按钮）一概不要。
 *
 * 正文搬的是子节点不是整块：正文在页面上是个 div，塞进这一行会自成一段，
 * 「昵称(时间): 」就单独留在上一行了。
 */
function formatReply(doc, article) {
	const body = firstPart(article, TWEET_TEXT);
	const media = ownParts(article, TWEET_MEDIA.join(','));
	if (!body && !media.length) return [];

	const line = doc.createElement('p');
	const name = tweetAuthorName(article);
	const time = tweetTime(article);
	const label = name && time ? `${name}(${time}): ` : name ? `${name}: ` : '';
	if (label) line.appendChild(doc.createTextNode(label));
	if (body) {
		const span = doc.createElement('span');
		span.setAttribute('class', REPLY_BODY_CLASS);
 ...
 * 后面 —— 主推之前的是上文，不算回复。
 *
 * 限定在同一条对话的容器里：回复列表底下还接着「更多推荐」那种不相干的推，
 * 它们在别的容器下。容器认不出来就不限定，宁可多收也别一条不收。
 *
...
function tweetReplies(doc, url) {
	...
	const blocks = tweets
		.slice(at + 1)
		.filter((el) => !conversation || conversation.contains?.(el))
		.flatMap((article) => formatReply(doc, article));
	if (!blocks.length) return [];

	// 分隔线（引擎自动加的）后面再报一下这是什么：底下这串是评论，不是正文的一部分
	const label = doc.createElement('p');
	label.appendChild(doc.createTextNode(REPLIES_LABEL));
	return [label, ...blocks];
}
...
export const SITE_RULES = [
	...
	},
	{
		...
		keepLineBreaks: [...TWEET_TEXT, `.${REPLY_BODY_CLASS}`],
	},
];
