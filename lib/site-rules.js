...
/** 推文分享链接的 query 都是附带参数；只清 status 页，不动搜索和个人页。 */
function normalizeTweetUrl(raw) {
	try {
		const url = new URL(raw);
		if (!/(^|\.)(x|twitter)\.com$/.test(url.hostname) ||
			!/^\/(?:[^/]+|i\/web)\/status\/\d+(?:\/|$)/.test(url.pathname)) return raw;
		url.search = '';
		return url.toString();
	} catch {
		return raw;
...
function tweetStamp(article, base) {
	...
	let url = '';
	try {
		url = href ? normalizeTweetUrl(new URL(href, base || 'https://x.com').toString()) : '';
	} catch {
		url = '';
	}
...
export const SITE_RULES = [
	...
	},
	{
		...
		normalizeUrl: normalizeTweetUrl,
		...
		root: [
			...
			// 长文章的普通 tweetText / dir="auto" 可能为空，正文在独立的富文本容器里。
			inFocal('.x-article-body'),
		...
		// 主推优先从当前 URL 的 snowflake 取日期，和回复共用换算及本地时区。
		publishedFrom: (_doc, url) => {
			const ms = timeFromStatusId(statusIdFromUrl(url));
			return ms ? localDate(ms) : '';
		},
		// 取不到 id 才用页面绝对时间。普通 status 链接也可能是 Reply / Views，
		// 文本只是「17」这样的互动数，不能再当发布时间。
		published: inFocal(TWEET_TIME[0]),
		// 长文章有独立标题；普通推文没有 h1，仍退回正文开头。
		title: inFocal('h1'),
