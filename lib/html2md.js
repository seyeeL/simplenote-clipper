...
//
// ctx.plain 是「移除 Markdown 格式」：标记全不写（# ** * ~~ ` > []()），只留文字和图片。
// 给排版坏掉的站点用 —— 公众号编辑器就有把整篇正文塞进一个 <h1> 的，照标题渲染会把
// 全文压成一行。
...
function render(node, ctx) {
	...
	if (HEADINGS[name]) {
		// plain 下标题当普通块处理：不光是去掉 #，更重要的是不能走 inline() ——
		// 标题里嵌着整篇正文时（公众号编辑器就这么干），那一步会把段落间的换行全压掉
		if (ctx.plain) return block(renderChildren(node, ctx));
	...
	switch (name) {
		...
		case 'BR':
			return '  \n';
		case 'HR':
			// plain 下退成一个段间空行：--- 也是 markdown 标记
			return ctx.plain ? '\n\n' : '\n\n---\n\n';
		case 'STRONG':
		case 'B': {
			const text = inline(node, ctx);
			if (!text) return '';
			return ctx.plain ? text : `**${text}**`;
		}
		case 'EM':
		case 'I': {
			const text = inline(node, ctx);
		...
		case 'PRE': {
			...
			// plain 下不写围栏，但保留代码本身的换行 —— 那是内容不是格式
			if (ctx.plain) return `\n\n${text}\n\n`;
		...
		case 'A': {
			...
			if (ctx.plain || !href || href.startsWith('#') || href.startsWith('javascript:')) return text;
			return `[${text}](${href})`;
		}
		case 'IMG': {
...
/**
 * @param {*} node 正文根节点
 * @param {{plain?: boolean}} [options] plain：移除 Markdown 格式，只留文字和图片
 */
export function htmlToMarkdown(node, options = {}) {
	if (!node) return '';
	return normalizeMarkdown(render(node, { listDepth: 0, plain: options.plain === true }));
}
