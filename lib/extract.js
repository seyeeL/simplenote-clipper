...
function pickTitle(scope, doc, siteName, rule, root, author = '') {
	...
	if (!text && rule?.titleFromBody) text = titleFromBody(root);
	// 独立标题和正文开头都走同一层包装，推特长文章也保留「作者 on X: …」。
	if (text) {
		return rule.titleFormat
			? rule.titleFormat({ body: text, author, siteName })
			: text;
	}
...
export function extractArticle(doc, url = '', options = {}) {
	const sourceUrl = url || doc?.location?.href || '';
	const rule = ruleFor(sourceUrl);
	const href = rule?.normalizeUrl ? rule.normalizeUrl(sourceUrl) : sourceUrl;
