...
/**
 * 用户手填标签 + 可选的作者标签和来源站点标签。
 * 后两个由设置页的开关控制，默认不加。
 */
export function buildTags({ tags = [], author = '', url = '', withAuthor = false, withSite = false } = {}) {
	const extra = [];
	if (withAuthor) extra.push(authorTag(author));
	if (withSite) extra.push(domainTag(url));
	return normalizeTags([...normalizeTags(tags), ...extra]);
}
 ...
 *
 * `#` 前缀是可选的：勾了「移除 Markdown 格式」之后那一行没有井号，同样要去重。
 * 一字不差才动，所以放宽前缀不会误删正文第一段。
 */
export function stripDuplicateHeading(markdown, title) {
	...
	if (!match || singleLine(match[1]) !== key) return body;
 ...
 * 属性行末尾各留两个空格，否则 Simplenote 预览会把连续几行并成一段。
...
export function buildNoteContent({
	...
	// 每行末尾两个空格：markdown 的硬换行写法。Simplenote 预览会把连续行并成一段，
	// 桌面端一个空格还能断开，移动端必须两个
