...
长文章取独立标题和富文本正文，标题同样包装为 `作者 on X: 文章标题`；X / Twitter 的 `/status/…` 来源链接自动去掉 query 参数，
保存和展示不再带 `?s=46&t=…`，回复和引用推文的日期链接也一样。
...
标签 = 你手填的（或设置里的默认值 `clippings`）。已保存的自定义标签（包括留空）不受影响。
设置页的「其它默认标签」还有两个开关，勾上才会额外打标签，默认都关：
...
测试用 node 原生 test runner。`html2md` 等纯逻辑测试沿用 `tests/fake-dom.mjs` 的假节点；
X 的选择器和提取管线用 `linkedom` 跑离线 DOM 回归，不依赖网络或登录态。它只用于开发测试，
扩展运行时不加载这个依赖。
